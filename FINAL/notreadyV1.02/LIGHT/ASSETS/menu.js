...
async function showDLCMenu() {
    ...
    const dlcWin = blessed.box({
        ...
        content: t('DLC_CHECKING'),
        style: { border: { fg: COLORDEFAULT }, label: { fg: COLORDEFAULT, bold: true } }
    });

    screen.render();

    // 1. Pega a lista da pasta /DLC no repositório
    const treeUrl = `https://api.github.com/repos/lukzst/LIGHT/contents/DLC`;
    const getDLCCmd = `powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-RestMethod -Uri '${treeUrl}' | ConvertTo-Json -Compress"`;

    exec(getDLCCmd, (error, stdout) => {
        if (error) {
            dlcWin.setContent(t('UPDATE_ERROR'));
            return screen.render();
        }

        try {
            const remoteDlcs = JSON.parse(stdout);
            
            // Filtra apenas pastas dentro de /DLC
            const dlcFolders = Array.isArray(remoteDlcs) ? remoteDlcs.filter(f => f.type === 'dir') : [];

            if (dlcFolders.length === 0) {
                dlcWin.setContent(t('DLC_EMPTY'));
                screen.render();
            } else {
                dlcWin.setContent(""); // Limpa o "Checking..."
                
                const dlcList = blessed.list({
                    parent: dlcWin,
                    top: 1, left: 2, right: 2, bottom: 3,
                    keys: true, mouse: true, tags: true,
                    style: { selected: { bg: COLORDEFAULT, fg: 'black' } },
                    items: dlcFolders.map(d => {
                        const localPath = path.join(__dirname, '..', 'DLC', d.name);
                        const status = fs.existsSync(localPath) ? t('DLC_INSTALLED') : '{red-fg}[NOT INSTALLED]{/}';
                        return `${d.name.padEnd(20)} ${status}`;
                    })
                });
                ...
                dlcList.on('select', (item, index) => {
                    const selectedDlc = dlcFolders[index];
                    // Aqui você chamaria uma função de download similar ao Updater
                    // passando `version` como `DLC/${selectedDlc.name}`
                    downloadDLC(selectedDlc.name, dlcWin);
                });
            }
        } catch (e) {
            dlcWin.setContent("{center}FAILED TO PARSE DLC DATA{/center}");
        }
        screen.render();
    });

    screen.key(['escape'], function closeDLC() {
        screen.unkey('escape', closeDLC);
        bgOverlay.destroy();
        mainList.focus();
        screen.render();
    });
...
async function downloadDLC(dlcName, statusWin) {
    statusWin.setContent(`{center}\nINITIATING DOWNLOAD: ${dlcName}...{/center}`);
    screen.render();

    // Reutiliza a lógica do Updater, mas aponta para DLC/nome-da-dlc
    const targetUrl = `https://api.github.com/repos/lukzst/LIGHT/git/trees/main?recursive=1`;
    const cmd = `powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; (Invoke-RestMethod -Uri '${targetUrl}').tree | Where-Object {$_.path -like 'DLC/${dlcName}/*'} | ConvertTo-Json -Compress"`;
    ...
    exec(cmd, async (err, stdout) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const fileUrl = `https://raw.githubusercontent.com/lukzst/LIGHT/main/${file.path}`;

            statusWin.setContent(`{center}INSTALLING ${dlcName}\n[${"█".repeat(i)}] ${Math.round((i/files.length)*100)}%{/center}`);
            screen.render();

            if (!fs.existsSync(path.dirname(destPath))) fs.mkdirSync(path.dirname(destPath), { recursive: true });
            const dlCmd = `powershell -NoProfile -Command "iwr -Uri '${fileUrl}' -OutFile '${destPath}'"`;
            await new Promise(res => exec(dlCmd, res));
        }

        statusWin.setContent(`{center}\n{green-fg}${dlcName} INSTALLED SUCCESSFULLY!{/}\nPRESS [ENTER] TO REBOOT{/center}`);
        playsucesso();
...
mainList.on('select', (item) => {
    ...
    if (text.includes(t('MENU_DLCS'))) { playBeep2(); return showDLCMenu(); }
