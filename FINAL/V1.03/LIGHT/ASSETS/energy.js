...
const protocolosBase = [
    ...
    { id: 2, text: t('ENERGIA_PROTOCOL_2') },
    { id: 3, text: t('ENERGIA_PROTOCOL_3') },
    { id: 4, text: t('ENERGIA_PROTOCOL_4') }
];
...
const logBox = blessed.log({
    ...
    tags: true, label: t('ENERGIA_LOG'), border: { type: 'line' },
...
const statusBox = blessed.box({
    ...
    label: t('ENERGIA_STATUS'), border: { type: 'line' },
    style: { fg: 'green', border: { fg: 'green' } },
    tags: true
});
...
const menuBox = blessed.list({
    ...
    tags: true, label: t('ENERGIA_PROTOCOLS'), border: { type: 'line' },
...
function execGameOver(reason) {
    ...
    setTimeout(() => {
        ...
        const goBox = blessed.box({
            ...
            height: 'shrink',
            padding: 2,
            content: t('ENERGIA_GAME_OVER', { reason }),
            tags: true,
            border: {
                type: 'line',
                fg: 'red'
...
function updateStatus() {
    statusBox.setContent(t('ENERGIA_STATUS_DISPLAY', { 
        energia: energia, 
        sanidade: sanidade, 
        saude: saude, 
        passo: passoAtual 
    ...
    if (energia <= 0) execGameOver(t('ENERGIA_ENERGY_FAIL'));
    else if (sanidade <= 0) execGameOver(t('ENERGIA_SANITY_FAIL'));
    else if (saude <= 0) execGameOver(t('ENERGIA_HEALTH_FAIL'));
...
function startFinalAuth() {
    ...
    const overlay = blessed.box({
        ...
        border: 'line', label: t('ENERGIA_AUTH'),
    ...
    const display = blessed.box({
        ...
        content: t('ENERGIA_KEY', { code: finalCode, time: timeLeft })
    });
    ...
    const countdown = setInterval(() => {
        timeLeft--;
        display.setContent(t('ENERGIA_KEY', { code: finalCode, time: timeLeft }));
        if (timeLeft <= 0) {
            clearInterval(countdown);
            execGameOver(t('ENERGIA_AUTH_TIMEOUT'));
        }
        screen.render();
    }, 1000);
    ...
    inputField.on('submit', (val) => {
        ...
        if (val === finalCode) {
            ...
            execGameOver(t('ENERGIA_INVALID_KEY'));
        }
    });
...
menuBox.on('select', (item, index) => {
    ...
    if (success) {
        playsucesso();
        showFeedback(ART_CHECK, false);
        logBox.add(t('ENERGIA_OK', { step: passoAtual }));
    } else {
        playGameOverSound();
        showFeedback(ART_X, true);
        logBox.add(t('ENERGIA_FAIL'));
        passoAtual = 0;
        energia -= 15;
        saude -= 10;
    }
